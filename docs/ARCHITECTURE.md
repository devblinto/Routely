...
750 ms timeout — the page can never stay hidden if the config request fails. A redirect
...
7. **Redirect flicker is mitigated, not eliminated** on slow connections; the cloak times out
   at 750 ms in favour of showing the Control page. A configuration request slower than that
